...
const compileProblem = async (req, res) => {
    ...
    try {
        ...
        for (const index in problema.tests) {
           ...
           const { stdout, stderr } = await exec(`cd ./uploads/compile/  && echo ${problema.tests[index].input} > ${user_id}.txt && gcc ${user_id}.cpp -o ${user_id} && ./${user_id} < ${user_id}.txt && rm ${user_id}.txt`)
